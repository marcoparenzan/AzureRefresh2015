...
namespace AdventureWorks.DomainPersistence.DocumentDb
{
    public class Repository<T>
    {
        ...
        public async Task<PageViewModel<TResult>> QueryItemsAsync<TResult>(string sql, dynamic args = null, int maxItemCount = 10, string continuation = "")
        {
            ...
            var page = new PageViewModel<TResult>
            {
                Items = result.ToList()
                ,
        ...
        public async Task<PageViewModel<T>> GetPageAsync(Expression<Func<T, bool>> predicate, int maxItemCount, string continuation = "")
        {
            var options = new FeedOptions {
                MaxItemCount = maxItemCount
            };
            ...
            var query = Client.CreateDocumentQuery<T>(Collection.DocumentsLink, options)
                .Where(predicate)
                .AsDocumentQuery()
            ;
        ...
        public async Task<Document> UpdateItemAsync(Expression<Func<Document, bool>> predicate, T item)
        {
            ...
            {
                PreTriggerInclude = new List<string>(new string[] { "UpdateCustomerInfo" })
            };
